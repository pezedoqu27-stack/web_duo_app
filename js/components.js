...
const Components = {
  ...
  renderMovieDetail(movie, currentList) {
    ...
    if (!currentList) {
      ...
      const cheatedBy = movie.cheatedBy;
      const cheatedBadge = cheatedBy ? `<span class="cheated-badge ${cheatedBy}">💔 ${cheatedBy === 'male' ? 'Он' : 'Она'} согрешил(а)</span>` : '';
      ...
      actionsHtml = `
        ...
        <button class="btn-secondary" data-action="open-bets">
          🎰 Сделать ставку
        </button>
        ${movie.type !== 'movie' ? `
          ...
          <button class="btn-cheating" data-action="i-cheated">
            😈 Я согрешил(а)
          </button>
        ` : ''}
  ...
  updateStats() {
    ...
    // Render challenges
    if (typeof App !== 'undefined' && App.renderChallenges) {
